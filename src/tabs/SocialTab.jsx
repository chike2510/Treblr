...
// ═══════════════════════════════════════════════════════════════════════════
// LIVING INTERFACE PRIMITIVES
// ═══════════════════════════════════════════════════════════════════════════

// Ambient aurora backdrop, tinted per screen so each platform feels distinct
// while staying in the same visual family.
const Aurora = ({ c1 = '#7C6CFF', c2 = '#3FD3C6', c3 = '#FF6FA5' }) => (
...
const StatNumber = ({ value, format, className, style }) => {
...
const PlatformIcon = ({ pid, size = 44 }) => {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:size*0.28, overflow:'hidden', background:c.bg, border:'1px solid '+c.border, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:`0 4px 14px ${c.bg}` }}>
      {!err
        ? <img src={c.icon} alt={c.label} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={() => setErr(true)} />
        : <span style={{ fontSize:size*0.35, fontWeight:700, fontFamily:'var(--li-font-display)', color:c.color }}>{c.label[0]}</span>
      }
    </div>
  );
};
...
const NpcAvatar = ({ npc, size = 36 }) => {
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', flexShrink:0, background:(npc?.color||'#444')+'30', border:'1px solid rgba(255,255,255,0.08)', display:'flex', alignItems:'center', justifyContent:'center' }}>
      {file && !err
        ? <img src={`/assets/avatars/${file}`} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={() => setErr(true)} />
        : <span style={{ fontWeight:700, fontFamily:'var(--li-font-display)', fontSize:size*0.35, color:npc?.color||'#aaa' }}>{(npc?.name||'?')[0]}</span>
      }
    </div>
  );
};
  ...
  return (
    <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', flexShrink:0, background:account.color+'20', border:'1.5px solid '+account.color+'50', display:'flex', alignItems:'center', justifyContent:'center' }}>
      {!err
        ...
        : <span style={{ fontWeight:700, fontFamily:'var(--li-font-display)', fontSize:size*0.4, color:account.color }}>{account.name[0]}</span>
      }
    </div>
  );
};
...
const PlayerAvatar = ({ gs, size = 36, ring }) => (
  <div style={{ width:size, height:size, borderRadius:'50%', overflow:'hidden', flexShrink:0, background:'var(--surface-2)', border:`2px solid ${ring||'var(--li-accent)'}`, boxShadow:`0 0 0 3px ${(ring||'#7C6CFF')}22`, display:'flex', alignItems:'center', justifyContent:'center', transition:'box-shadow 300ms var(--li-ease-smooth)' }}>
    {gs.avatarUrl
...
const ChartCard = ({ chartCard }) => {
  ...
  return (
    <div className="li-glass" style={{ overflow:'hidden', marginTop:10, borderRadius:14 }}>
      <div style={{ background:`linear-gradient(135deg, ${cfg.bg}, ${cfg.bg}cc)`, padding:'11px 14px' }}>
        <div style={{ fontSize:9, fontStyle:'italic', color:'rgba(255,255,255,0.75)', marginBottom:3, letterSpacing:0.5 }}>hillboard</div>
        <div style={{ display:'flex', alignItems:'baseline', gap:5 }}>
          <span style={{ fontFamily:'var(--li-font-display)', fontSize:20, fontWeight:700, color:'#fff', letterSpacing:-0.5 }}>{cfg.title}</span>
          <span style={{ fontFamily:'var(--li-font-display)', fontSize:20, fontWeight:700, color:'rgba(255,255,255,0.6)', letterSpacing:-0.5 }}>{cfg.num}</span>
        </div>
      </div>
      <div>
        {(chartCard.entries||[]).map((entry, i) => (
          <div key={i} style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 14px', borderBottom: i < chartCard.entries.length-1 ? '1px solid var(--li-glass-border)' : 'none' }}>
        ...
        <div className="li-scene-content">
          ...
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ height:4, background:'var(--li-glass-bg)', borderRadius:4, marginTop:14, overflow:'hidden', border:'1px solid var(--li-glass-border)' }}>
              <div style={{ height:'100%', width:Math.min(100, totalFollowers/10000)+'%', background:'linear-gradient(90deg,var(--li-accent),#3FD3C6)', borderRadius:4, transition:'width 700ms var(--li-ease-smooth)', boxShadow:'0 0 10px var(--li-accent-glow)' }} />
            </div>
          </div>

          {/* SE dots */}
          <div className="li-glass li-stagger" style={{ '--i':0, display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 16px', marginBottom:16 }}>
            <span style={{ fontSize:12, color:'var(--text-secondary)', fontWeight:500 }}>Social Energy</span>
            <div style={{ display:'flex', gap:6 }}>
              {Array.from({ length:gs.maxSe||7 }).map((_,i) => (
                <div key={i} className={i<seLeft ? 'li-dot-live' : ''} style={{ width:9, height:9, borderRadius:'50%', background:i<seLeft?'var(--li-accent-lt)':'var(--li-glass-border)', transition:'background 250ms var(--li-ease-smooth)' }} />
              ))}
            </div>
          </div>

          {/* Platform list */}
          <div className="li-glass" style={{ padding:0, overflow:'hidden' }}>
            {PLATFORM_ORDER.map((pid, idx) => {
              const c   = PC[pid]||{};
              const count = platforms[pid]||0;
              const isAuto = pid==='soundify';
              return (
                <Magnetic key={pid} strength={6}
                  onClick={() => { setActivePlatform(pid); setChirpScreen('feed'); setInstapicScreen('feed'); setRythmScreen('profile'); }}
                  className="li-row li-stagger"
                  style={{ '--i':idx+1, display:'flex', alignItems:'center', gap:14, padding:'14px 16px', borderBottom:idx<PLATFORM_ORDER.length-1?'1px solid var(--li-glass-border)':'none' }}
                >
                  <PlatformIcon pid={pid} size={44} />
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:700, fontSize:14, fontFamily:'var(--li-font-display)' }}>{c.label}</div>
  ...
  if (activePlatform==='soundify') {
    ...
    return (
      <div className="tab-content li-scene">
        ...
        {/* Search bar */}
        <div className="li-glass" style={{ display:'flex', alignItems:'center', gap:10, borderRadius:22, padding:'11px 16px', marginBottom:20 }}>
        ...
        {/* Filter chips */}
        <div className="soc-scroll-x" style={{ gap:8, marginBottom:20 }}>
          {['All','Music','Podcasts','Audiobooks','Live Events'].map((t,i) => (
            <div key={t} className="soc-pill" style={{ flexShrink:0, padding:'6px 14px', background:i===0?c.color:'var(--li-glass-bg)', border:i===0?'none':'1px solid var(--li-glass-border)', color:i===0?'#000':'var(--text-muted)', fontSize:12, fontWeight:i===0?700:400 }}>{t}</div>
          ))}
        </div>

        {/* Featured banner */}
        <div className="li-glass" style={{ height:160, overflow:'hidden', background:'linear-gradient(135deg,#1a1a2e,#16213e)', marginBottom:20, position:'relative', display:'flex', alignItems:'flex-end', padding:14 }}>
          ...
          <div>
            <div style={{ fontFamily:'var(--li-font-display)', fontSize:22, fontWeight:700 }}>{gs.stageName||'You'}</div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <div style={{ fontWeight:700, fontSize:15, fontFamily:'var(--li-font-display)' }}>Popular Artists</div>
          <div style={{ fontSize:12, color:'var(--text-muted)', cursor:'pointer' }}>SEE ALL</div>
        </div>
        <div className="soc-scroll-x" style={{ gap:16, paddingBottom:12, marginBottom:20 }}>
          <div style={{ flexShrink:0, textAlign:'center', width:72 }}>
            <div style={{ width:72, height:72, borderRadius:'50%', overflow:'hidden', background:'var(--surface-2)', margin:'0 auto 6px', border:'1px solid var(--li-glass-border)' }}>
              {gs.avatarUrl ? <img src={gs.avatarUrl} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, fontFamily:'var(--li-font-display)' }}>{(gs.stageName||'?')[0]}</div>}
            </div>
        ...
        {/* Made For You */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <div style={{ fontWeight:700, fontSize:15, fontFamily:'var(--li-font-display)' }}>Made For {gs.stageName}</div>
          <div style={{ fontSize:12, color:'var(--text-muted)', cursor:'pointer' }}>SEE ALL</div>
        </div>
        <div className="soc-scroll-x" style={{ gap:12, paddingBottom:8 }}>
          {playlists.map(pl => (
            <div key={pl.name} style={{ flexShrink:0, width:120 }}>
              <div className="li-glass" style={{ width:120, height:120, background:`linear-gradient(135deg,${pl.color}22,${pl.color}55)`, borderColor:pl.color+'44', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:6 }}>
                <div style={{ fontFamily:'var(--li-font-display)', fontSize:13, fontWeight:700, color:pl.color, textAlign:'center', padding:8 }}>{pl.name}</div>
              </div>
        ...
        <div className="li-glass" style={{ marginTop:20, padding:16 }}>
          ...
          <div style={{ display:'flex', gap:14, alignItems:'center', marginBottom:14 }}>
            <div style={{ width:80, height:80, borderRadius:'50%', overflow:'hidden', background:'var(--surface-2)' }}>
              {gs.avatarUrl ? <img src={gs.avatarUrl} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, fontFamily:'var(--li-font-display)' }}>{(gs.stageName||'?')[0]}</div>}
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:'var(--li-font-display)', fontSize:20, fontWeight:700 }}>{gs.stageName?.toUpperCase()}</div>
              <div style={{ fontSize:13, color:'var(--text-muted)' }}>{fmt(followers)} monthly listeners</div>
              <div style={{ display:'flex', gap:10, marginTop:8 }}>
                <button className="soc-pill" style={{ padding:'7px 18px', border:'1px solid rgba(255,255,255,0.4)', background:'none', color:'#fff', fontSize:12 }}>Follow</button>
                <button className="soc-pill" style={{ padding:'7px 14px', border:'1px solid rgba(255,255,255,0.2)', background:'none', color:'#fff', fontSize:12, fontWeight:400 }}>···</button>
                <Magnetic strength={8} style={{ marginLeft:'auto', width:38, height:38, borderRadius:'50%', background:c.color, display:'flex', alignItems:'center', justifyContent:'center' }}>
          ...
          {/* Tabs */}
          <div style={{ display:'flex', gap:20, borderBottom:'1px solid var(--li-glass-border)', marginBottom:14 }}>
          ...
          {releasedTracks.length===0 ? (
            ...
            <div>
              ...
              {releasedTracks.slice(0,5).map((track, idx) => (
                <div key={track.id} className="li-row" style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 0', borderBottom:'1px solid var(--li-glass-border)' }}>
        ...
        <div className="tab-content li-scene">
          ...
          <div style={{ display:'flex', gap:24, alignItems:'center', marginBottom:16 }}>
            ...
            <div style={{ flex:1, display:'flex', justifyContent:'space-around', textAlign:'center' }}>
              ...
              <div><StatNumber value={followers} className="li-count" style={{ fontWeight:700, fontSize:17, color:c.color, display:'block' }} /><div style={{ fontSize:11, color:'var(--text-muted)' }}>Followers</div></div>
          ...
          <div style={{ display:'flex', gap:8, marginBottom:16 }}>
            <button onClick={() => doPost('instapic', 1)} disabled={seLeft<1} className="soc-glass-btn" style={{ flex:1, padding:'8px 0', fontSize:12, fontWeight:700, color:seLeft>=1?'#fff':'var(--text-muted)' }}>Edit Profile</button>
            <button onClick={() => doPost('instapic', 1)} disabled={seLeft<1} className="soc-glass-btn" style={{ flex:1, padding:'8px 0', fontSize:12, fontWeight:700, color:seLeft>=1?'#fff':'var(--text-muted)' }}>Share Profile</button>
            <button onClick={() => doPost('instapic', 1)} disabled={seLeft<1} className="soc-glass-btn" style={{ padding:'8px 14px', color:seLeft>=1?'#fff':'var(--text-muted)' }}>
          ...
          {/* Post grid */}
          <div style={{ display:'flex', borderBottom:'1px solid var(--li-glass-border)', marginBottom:2, gap:24, justifyContent:'center' }}>
            {[
              <svg key="grid" viewBox="0 0 24 24" style={{ width:20, height:20, fill:'none', stroke:'currentColor', strokeWidth:1.5 }}><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>,
              <svg key="reel" viewBox="0 0 24 24" style={{ width:20, height:20, fill:'none', stroke:'currentColor', strokeWidth:1.5 }}><rect x="2" y="2" width="20" height="20" rx="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="2" y1="7" x2="7" y2="7"/><line x1="2" y1="17" x2="7" y2="17"/><line x1="17" y1="17" x2="22" y2="17"/><line x1="17" y1="7" x2="22" y2="7"/></svg>,
              <svg key="tag" viewBox="0 0 24 24" style={{ width:20, height:20, fill:'none', stroke:'currentColor', strokeWidth:1.5 }}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
            ].map((icon, i) => (
              <div key={i} className="soc-tab-underline on" style={{ padding:'10px 12px', color:i===0?'#fff':'var(--text-muted)' }}>{icon}</div>
            ))}
          </div>
          ...
          {releasedTracks.length===0 ? (
            <div style={{ textAlign:'center', padding:'40px 20px', color:'var(--text-muted)' }}>
              ...
              <button onClick={() => doPost('instapic', 1)} disabled={seLeft<1} className="soc-pill" style={{ marginTop:14, padding:'10px 28px', background:c.color, color:'#fff', fontSize:13 }}>+ Create Post</button>
            </div>
          ) : (
            <>
              ...
              <button onClick={() => doPost('instapic', 1)} disabled={seLeft<1} className="soc-pill" style={{ width:'100%', marginTop:12, padding:'10px 0', background:seLeft>=1?c.bg:'var(--li-glass-bg)', border:'1px solid '+(seLeft>=1?c.border:'var(--li-glass-border)'), color:seLeft>=1?c.color:'var(--text-muted)', fontSize:12 }}>
                + New Post · 1 SE
              </button>
            </>
          )}
      ...
      <div className="tab-content li-scene">
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          ...
          <div style={{ display:'flex', gap:16, alignItems:'center' }}>
            <svg className="soc-icon-btn" viewBox="0 0 24 24" style={{ width:22, height:22, fill:'none', stroke:'#fff', strokeWidth:1.5 }}><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
            <svg className="soc-icon-btn" viewBox="0 0 24 24" style={{ width:22, height:22, fill:'none', stroke:'#fff', strokeWidth:1.5 }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
          </div>
        </div>
        ...
        <button onClick={() => setInstapicScreen('profile')} className="soc-glass-btn" style={{ width:'100%', padding:'9px 0', marginBottom:14, color:'var(--text-secondary)', fontSize:12, fontWeight:700 }}>
          View Your Profile
        </button>

        {/* NPC posts */}
        {npcPosts.map((npc, i) => (
          <div key={npc.id} className="li-stagger" style={{ '--i':i, marginBottom:20 }}>
            ...
            <div className="li-glass" style={{ height:260, overflow:'hidden', margin:'0 -1px', borderRadius:16 }}>
            ...
            <div style={{ padding:'10px 0 0' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                <div style={{ display:'flex', gap:14 }}>
                  <svg className="soc-icon-btn" viewBox="0 0 24 24" style={{ width:22, height:22, fill:'none', stroke:'#fff', strokeWidth:1.5 }}><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                  <svg className="soc-icon-btn" viewBox="0 0 24 24" style={{ width:22, height:22, fill:'none', stroke:'#fff', strokeWidth:1.5 }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                  <svg className="soc-icon-btn" viewBox="0 0 24 24" style={{ width:22, height:22, fill:'none', stroke:'#fff', strokeWidth:1.5 }}><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                </div>
  ...
  if (activePlatform==='chirp') {
    ...
    if (chirpScreen==='compose') {
      ...
      return (
        <div className="tab-content li-scene">
          ...
          <div style={{ height:1, background:'var(--li-glass-border)', margin:'14px 0' }} />
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div style={{ width:24, height:24, borderRadius:'50%', border:`2px solid ${charLeft<20?'var(--accent-red)':c.color}` }}>
                <svg viewBox="0 0 24 24" style={{ width:20, height:20 }}>
                  <circle cx="12" cy="12" r="10" fill="none" stroke="var(--li-glass-border)" strokeWidth="3"/>
                  <circle cx="12" cy="12" r="10" fill="none" stroke={c.color} strokeWidth="3"
                    strokeDasharray={`${((280-charLeft)/280)*63} 63`} strokeLinecap="round" transform="rotate(-90 12 12)" style={{ transition:'stroke-dasharray 200ms var(--li-ease-smooth)' }}/>
                </svg>
              </div>
            </div>
          </div>
          ...
          {/* SE cost note */}
          <div className="li-glass" style={{ marginTop:16, padding:'10px 14px', fontSize:12, color:'var(--text-muted)' }}>
    ...
    if (chirpScreen==='profile') {
      return (
        <div className="tab-content li-scene">
          ...
          {/* Banner */}
          <div className="li-glass" style={{ height:100, background:'linear-gradient(135deg,#1a1a2e,#16213e,#0f3460)', margin:'12px -1px 0', position:'relative', borderRadius:16 }}>
            <button className="soc-glass-btn" style={{ position:'absolute', bottom:8, right:12, padding:'5px 12px', color:'#fff', fontSize:11 }}>Edit Banner</button>
          </div>
          ...
          <div style={{ position:'relative', marginBottom:8 }}>
            ...
            <div style={{ paddingTop:32, display:'flex', justifyContent:'flex-end', gap:8 }}>
              <button className="soc-pill" style={{ padding:'6px 14px', background:'none', border:'1px solid rgba(255,255,255,0.4)', color:'#fff', fontSize:12 }}>Edit profile</button>
            </div>
          </div>
          ...
          {followers===0 ? (
            <div style={{ textAlign:'center', padding:'32px 0', color:'var(--text-muted)', fontSize:13 }}>
              ...
              <button onClick={() => setChirpScreen('compose')} disabled={seLeft<1} className="soc-pill"
                style={{ marginTop:14, padding:'10px 24px', background:seLeft>=1?c.color:'var(--li-glass-bg)', color:seLeft>=1?'#fff':'var(--text-muted)', fontSize:13 }}>
            ...
            <>
              {releasedTracks.slice(0,3).map((track, i) => (
                <div key={track.id} className="li-row li-stagger" style={{ '--i':i, padding:'12px 0', borderBottom:'1px solid var(--li-glass-border)', fontSize:13 }}>
                  <div style={{ display:'flex', gap:10, marginBottom:6, alignItems:'center' }}>
                    <PlayerAvatar gs={gs} size={32} ring={c.color} />
                    <div>
    ...
    return (
      <div className="tab-content li-scene">
        ...
        {/* Header */}
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          <button onClick={() => setActivePlatform(null)} className="soc-icon-btn" style={{ background:'none', border:'none', cursor:'pointer', padding:0 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <span style={{ fontFamily:'var(--li-font-display)', fontSize:18, fontWeight:700, color:c.color }}>Chirp</span>
          </div>
          <button onClick={() => setChirpScreen('profile')} className="soc-icon-btn" style={{ background:'none', border:'none', padding:0 }}>
        ...
        {chirpFeed.map((item, i) => (
          <div key={i} className="li-stagger" style={{ '--i':i, padding:'14px 0', borderBottom:'1px solid var(--li-glass-border)', display:'flex', gap:12 }}>
        ...
        {/* Compose FAB */}
        <Magnetic strength={10} onClick={() => setChirpScreen('compose')} className="soc-fab"
          style={{ position:'fixed', bottom:80, right:16, width:52, height:52, borderRadius:'50%', background:c.color, display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, boxShadow:`0 6px 20px ${c.color}66` }}>
  ...
  if (activePlatform==='vidtube') {
    return (
      <div className="tab-content li-scene">
        ...
        <div className="li-scene-content">
        <BackBtn onBack={() => setActivePlatform(null)} />
        <div className="li-glass" style={{ height:90, background:'linear-gradient(135deg,#1a1a2e,#16213e,#0f3460)', marginBottom:0, display:'flex', alignItems:'center', justifyContent:'center', color:'rgba(255,255,255,0.15)', fontSize:12, letterSpacing:2 }}>CHANNEL BANNER</div>
  ...
  if (activePlatform==='rhythmtok') {
    return (
      <div className="tab-content li-scene">
        ...
        {/* Profile — avatar + stats side by side */}
      ...
      <div className="tab-content li-scene">
        ...
        <div className="li-glass" style={{ padding:16, marginBottom:16 }}>
          <div style={{ display:'flex', gap:14, alignItems:'center', marginBottom:14 }}>
            ...
            <button onClick={() => doPost('wavelog', 1)} disabled={seLeft<1} className="soc-pill"
              style={{ padding:'8px 14px', background:seLeft>=1?c.color:'var(--li-glass-bg)', color:seLeft>=1?'#fff':'var(--text-muted)', fontSize:12 }}>
              Share · 1 SE
            </button>
          </div>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, textAlign:'center' }}>
            <div><StatNumber value={followers} className="li-count" style={{ fontWeight:700, color:c.color, fontSize:15, display:'block' }} /><div style={{ color:'var(--text-muted)', fontSize:10, letterSpacing:1 }}>FOLLOWERS</div></div>
        ...
        <button onClick={() => doPost('wavelog', 1)} disabled={seLeft<1} className="soc-pill"
          style={{ width:'100%', padding:'12px 0', marginBottom:16, background:seLeft>=1?c.bg:'var(--li-glass-bg)', border:'1px solid '+(seLeft>=1?c.border:'var(--li-glass-border)'), color:seLeft>=1?c.color:'var(--text-muted)', fontSize:13 }}>
        ...
        {releasedTracks.length===0 ? (
          ...
          <div className="li-glass" style={{ padding:'0 16px' }}>
            {releasedTracks.map((track, idx) => (
              <div key={track.id} className="li-row" style={{ display:'flex', gap:12, padding:'12px 0', borderBottom:idx<releasedTracks.length-1?'1px solid var(--li-glass-border)':'none', alignItems:'center' }}>
                <div style={{ width:42, height:42, borderRadius:8, overflow:'hidden', flexShrink:0, background:'var(--surface-2)' }}>
                  {track.coverArt ? <img src={track.coverArt} style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <img src={`/assets/covers/${COVER_POOL[(idx+gs.totalWeeks)%COVER_POOL.length]}`} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e=>e.target.style.display='none'} />}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
