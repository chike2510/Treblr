...
export const calcWeeklyStreamIncome = (gs) => {
  ...
  let total = 0;
  // Tracks bundled into an EP/album (inProject) earn through the project's own
  // income calc in weekEngine.js — excluding them here prevents double-paying
  // the same song once as a single AND again as part of the project.
  const released = gs.catalog.filter(t => t.released && t.releaseWeek != null && !t.inProject);
